...
namespace PhpDomUtils\Tests;

use DOMImplementation;
use PhpDomUtils\PhpDomUtils;
...
class PhpDomUtilsTest extends \PHPUnit_Framework_TestCase
{
     ...
     * @test
     */
    public function fillDowWithSpecificDocType()
    {

        $domImplementation = new DOMImplementation();
        $docType = $domImplementation->createDocumentType('graph', '', 'graph.dtd');
        $dom = $domImplementation->createDocument('','',$docType);

        PhpDomUtils::fillDom($dom, $this->getArrayFixture());

        $expectedOutput = '<?xml version="1.0"?><!DOCTYPE graph SYSTEM "graph.dtd"><nodes><node>text1</node><node>text2</node></nodes>';
